...
RE_sales AS (
  ...
  WHERE 
    ...
    AND msg_value:execute_msg:ledger_proxy:msg:execute_order:order:order:maker_asset:info:nft:contract_addr::string 
    = 'terra1jp5fjj7rlc0erw4z3qr5zuvmg2w49pfzyhvsnk' --monkeez contract
    AND tx_status = 'SUCCEEDED'
),
...
daily_sales AS (
  SELECT 
    ...
    SUM(luna) AS luna_volume_daily, 
    COUNT(tx_id) AS tx_volume 
  FROM 
    All_sales 
  GROUP BY 
    daily 
...
weekly_sales AS (
  SELECT 
    ...
    SUM(luna) AS luna_volume_weekly, 
    COUNT(tx_id) AS tx_volume 
  FROM 
    All_sales 
  GROUP BY 
...
monthly_sales AS (
  SELECT 
    ...
    SUM(luna) AS luna_volume_monthly, 
    COUNT(tx_id) AS tx_volume 
  FROM 
    All_sales 
  GROUP BY 
...
SELECT 
  weekly AS weekly, 
  luna_volume_weekly AS luna_volume_weekly, 
  NULL AS luna_volume_daily, 
  NULL AS luna_volume_monthly, 
  tx_volume AS tx_volume, 
  NULL AS daily, 
  NULL AS monthly 
FROM 
  weekly_sales
...
SELECT 
  NULL AS weekly, 
  NULL AS luna_volume_weekly, 
  luna_volume_daily AS luna_volume_daily, 
  NULL AS luna_volume_monthly, 
  tx_volume AS tx_volume, 
  daily AS daily, 
  NULL AS monthly 
...
SELECT 
  ...
  NULL AS luna_volume_weekly, 
  NULL AS luna_volume_daily, 
  luna_volume_monthly AS luna_volume_monthly, 
  tx_volume AS tx_volume, 
  NULL AS daily, 
  monthly AS monthly 
FROM 
